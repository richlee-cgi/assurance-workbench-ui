...
The installer uses the current checkout when run from a cloned repo. If you have not cloned the repo yet, use the one-line installer.
...
`Skip preset expansion` is enabled by default in the Workbench. This keeps preset runs focused on the topic and any explicit additional queries you enter, while still using the preset's source defaults. Turn it off when you deliberately want a preset such as `architecture`, `delivery` or `risk` to add its own broader generated search terms.
